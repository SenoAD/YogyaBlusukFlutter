...
class EditVacationPlanPage extends StatefulWidget {
  final VacationPlanRepository vacationPlanRepository;
  final PlaceRepository placeRepository;
  final int? userId;
  final VacationPlan? vacationPlan;

  EditVacationPlanPage({required this.vacationPlanRepository, required this.placeRepository, required this.userId, required this.vacationPlan});

  @override
  _EditVacationPlanPageState createState() => _EditVacationPlanPageState();
}

class _EditVacationPlanPageState extends State<EditVacationPlanPage> {
  ...
  void initState() {
    super.initState();
    if (widget.vacationPlan != null) {
      fetchDataFromAPI(widget.vacationPlan!.planId??0);
      // Initialize _planNameController and _descriptionController with vacationPlan values
      _planNameController.text = widget.vacationPlan!.name ?? '';
      _descriptionController.text = widget.vacationPlan!.description ?? '';

      // Initialize _selectedPlaces and _planItem with vacationPlan values
      // if (widget.vacationPlan!.planItems.isNotEmpty) {
      //   for (var item in widget.vacationPlan!.planItems) {
      //     // Assuming placeRepository has a method to get a place by its ID
      //     Place? place = widget.placeRepository.getPlaceById(item.placeID);
      //     if (place != null) {
      //       _selectedPlaces.add(PlaceWithDate(place: place, date: item.planDate ?? DateTime.now()));
      //       _planItem.add(item);
      //     }
      //   }
      // }
  ...
  Future<void> fetchDataFromAPI(int planId) async {
    try {
      final VacationPlan vacationPlan = await widget.vacationPlanRepository.GetVacationPlanByPlanIdUC(planId);
      for (var item in vacationPlan.planItem) {
        var place = await widget.placeRepository.getPlacebyIdUC(item.placeId);
        _selectedPlaces.add(PlaceWithDate(place: place, date: item.planDate));
        _planItem.add(item);
      }
      setState(() {
        // Update the state after populating _selectedPlaces
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Edit ${(widget.vacationPlan?.name??"")}'),
        actions: [
          IconButton(
            onPressed: () async {
